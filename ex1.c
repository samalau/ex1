...
int main() {
	...
	printf("Number with bit %d toggled: %d", position, number);
	...
	bit = number & 1; printf("%d", bit);  // checks LSB -- 1 is odd, 0 is even
	...
	// used only in #5
	int firstNum, secondNum, sum, result;
	...
	result = (posit11 << 3) | (posit7 << 2) | (posit5 << 1) | posit3;
	printf("The 3, 5, 7, 11 bits are: %d", result);
